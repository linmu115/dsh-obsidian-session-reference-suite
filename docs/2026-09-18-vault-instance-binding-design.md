...
| VB11 | 保留后续 DSH → Obsidian 专门操作管道的可扩展性。 | 现在只预留身份、路由、能力注册与请求结果合同；笔记、样式、模板、面板和插件注册功能以后实施。 |
...
未来 Obsidian 操作插件可向 Bridge 注册带命名空间和版本的操作能力，DSH 工具通过同一绑定及路由调用。Bridge 负责目标、连接、请求身份和结果关联；操作插件调用 Obsidian 的笔记、模板、样式、面板或插件生命周期接口。

每个 Vault 分别声明已安装且启用的能力。操作请求必须确定 vaultId，并绑定当前绑定修订；配对本身不等同自动启用全部操作。请求支持幂等或明确声明不可重试，注销扩展释放对应能力而不修改绑定。

本次只预留登记、能力查询、调用和结果的接口位置，不实现 LLM 创建插件、执行脚本、写模板或修改面板样式，也不新增必装的操作管道插件。
...
- 用合成操作扩展验证可注册、查询、按 Vault 调用和注销；不以此冒充未来写笔记／样式／插件功能已实现。
