...
以下保留当时分期边界：第一阶段可以整理目标上下文与接口位置，保留已经存在的身份字段，避免新通道写死在普通贴纸内部；不提前实现第二阶段的多 Vault 行为。已有维护接入的兼容调整可以随重构发生，新的公开注册与绑定页另阶段验收。

## 实施边界

此前用户要求先核实并等待其下令。2026-09-18 用户已明确下令按需求和执行规划施工，并要求由 Astra 子代理（思考强度不超过 high）分工执行。前三阶段内部能力已完成本地合成验收；当前按 [[DEC-single-bridge-product]] 补齐单桥产品交付，详见[实施计划](../../../2026-09-18-bridge-implementation-plan.md)。代码、合成测试与本地构件按阶段推进，真实数据迁移和应用部署的完成情况另行记录。
