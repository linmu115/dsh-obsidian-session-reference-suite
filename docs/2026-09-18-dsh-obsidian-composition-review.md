...
基础连接与引用、关联笔记、贴纸的既有协议依赖应分开。2026-09-18 后续用户修订：未来专门操作通道直接提供操作权限，不要求逐 Vault 操作能力声明、识别或协商；这一修订不删除已有引用链路的兼容性检查，也不新增运行插件。
...
2026-09-18 当前技术讨论的后续修订：用户要求桥交付操作 skill，提示 DSH 优先走绑定 Vault 的官方 CLI。下述 HTTP / Plugin API 执行链保留为 CLI 未覆盖功能的建议补充路径，不再表示所有操作默认经过自建 HTTP 接口；当前路线及实现边界见 [[DEC-obsidian-operation-route]]。

两侧都需要相应能力，但可以分别放在现有两侧 Bridge 内：

1. DSH 侧向 LLM 暴露有名称和结构化参数的工具，通过绑定 Vault 的直接操作权限发出请求并解释结果；不先识别或查询该 Vault 的操作能力目录。
...
按后续单桥产品决定及本次命名澄清，执行器放在现有 Obsidian 侧 Bridge 内，DSH 工具放在现有 DSH Bridge 内，复用已经完成的配对、发现和路由，不新增专用操作插件安装项。Obsidian Companion 是现有 Obsidian 侧 DeepHarness Bridge 的旧称。纯笔记操作不必经过 Core 引用流程或 Maintenance；通道业务尚未实现。
